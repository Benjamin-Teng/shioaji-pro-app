...
- 深度熱圖以小視窗冷開啟時直接填滿版面，不需手動調整視窗（#106）。
- 模擬環境 1.7.5 的股票持倉昨餘數量單位待上游確認（Sinotrade/Shioaji#233）：持倉表顯示「待確認」而非推算張／股，尚未取得伺服器資訊時亦顯示待確認；其他數量欄位保留 SDK 原值。伺服器資訊只沿用既有查詢並依請求順序更新，不新增輪詢（#107）。
